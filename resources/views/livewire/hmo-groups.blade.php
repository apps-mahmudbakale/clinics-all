<div>
    <div id="users_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                            ...
                            <td class="text-right">
                                <span class="dropdown ml-1">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a class="dropdown-item" href="" wire:click.prevent="selectHmo({{ $hmo->id }})">
                                            Edit
                                        </a>
                                        <button class="dropdown-item" id="del{{ $hmo->id }}" data-value="{{ $hmo->id }}">
                                            Delete
                                        </button>
                                    </div>
                                </span>
                                <script>
                                    document.querySelector('#del{{ $hmo->id }}').addEventListener('click', function(e) {
                                ...
                                <form id="del#{{ $hmo->id }}"
                                    action="{{ route('app.hmos.destroy', $hmo->id) }}" method="POST"
        ...
        <div class="row">
            <div class="col-sm-12 col-md-5">
                <div class="dataTables_info" id="users_info" role="status" aria-live="polite">Showing <b>{{ $hmos->firstItem() }}</b> to
                    <b>{{ $hmos->lastItem() }}</b> out of <b>{{ $hmos->total() }}</b> entries</div>
            </div>
    ...
    @push('body-scripts')
    @once
    window.addEventListener('HmoEditModal', function() {
        $('#edit-hmo-modal').modal('show');
    });
    @endonce
@endpush
@include('hmos.modals.new')
@include('hmos.modals.edit')
</div>
